const clubs = [
    {
        '_id': '1',
        'name': 'Arsenal F.C',
        'image': '/images/ArsenalEmblema.png',
        'description':
        "Arsenal Football Club is a professional football club based in Islington, London, England. Arsenal plays in the Premier League, the top flight of English football. The club has won 13 league titles (including one unbeaten title), a record 14 FA Cups, two League Cups, 16 FA Community Shields, the League Centenary Trophy, one European Cup Winners' Cup, and one Inter-Cities Fairs Cup. In terms of trophies won, it is the third-most successful club in English football.",
        'country': 'England',
        'city': 'London',
        'liga': 'EPL',
        'founded': 1886,
        'Coach': 'Mikele Arteta',
        'rating': 4.5,
        'stadion': 'Emirates',
        'webSait': 'https://www.arsenal.com/'
    },
    {
        '_id': '2',
        'name': 'Aston Villa F.C.',
        'image': '/images/AstonVillaEmblema.png',
        'description':
        "Aston Villa Football Club is an English professional football club based in Aston, Birmingham. The club competes in the Premier League, the top tier of the English football league system. Founded in 1874, they have played at their home ground, Villa Park, since 1897.",
        'country': 'England',
        'city': 'London',
        'liga': 'EPL',
        'founded': 1874,
        'Coach': 'Steven Gerrard',
        'rating': 4,
        'stadion': 'Villa Park',
        'webSait': 'https://www.avfc.co.uk'
    },
    {
        '_id': '3',
        'name': 'Brentford F.C',
        'image': '/images/BrentfordEmblema.png',
        'description':
        "Brentford Football Club is a professional football club based in Brentford, West London, England. They currently compete in the Premier League, the highest tier of English football, having gained promotion via the playoffs at the end of the 2020–21 Championship season. Nicknamed 'the Bees'.",
        'country': 'England',
        'city': 'Brentford',
        'liga': 'EPL',
        'founded': 1874,
        'Coach': 'Thomas Frank',
        'rating': 3.5,
        'stadion': 'Brentford Community Stadium',
        'webSait': 'https://www.brentfordfc.com/'
    },
    {
        '_id': '4',
        'name': 'Brighton F.C.',
        'image': '/images/BrightonEmblema.png',
        'description':
        "Brighton & Hove Albion Football Club (/ˈbraɪtən ... ˈhoʊv/), commonly referred to simply as Brighton, is an English professional football club based in Brighton and Hove. They compete in the Premier League, the top tier of the English football league system. The club's home ground is the 31,800-capacity Falmer Stadium, situated in Falmer to the north east of the city.",
        'country': 'England',
        'city': 'Brighton',
        'liga': 'EPL',
        'founded': 1901,
        'Coach': 'Graham Potter',
        'rating': 3.5,
        'stadion': 'Falmer Stadium',
        'webSait': 'https://www.brightonandhovealbion.com/'
    },
    {
        '_id': '5',
        'name': 'Burnley F.C.',
        'image': '/images/BurnleyEmblema.png',
        'description':
        "Burnley Football Club (/ˈbɜːrnli/) is an English association football club based in Burnley, Lancashire, that currently competes in the Premier League, the top tier of English football. Founded on 18 May 1882, it was one of the first to become professional (in 1883), and subsequently put pressure on the Football Association to permit payments to players.",
        'country': 'England',
        'city': 'Burnley',
        'liga': 'EPL',
        'founded': 1882,
        'Coach': 'Sean Dyche',
        'rating': 3.5,
        'stadion': 'Turf Moor',
        'webSait': 'https://www.burnleyfootballclub.com/'
    },
    {
        '_id': '6',
        'name': 'Chelsea F.C',
        'image': '/images/ChelseaEmblema.png',
        'description':
        "Chelsea Football Club is an English professional football club based in Fulham, West London. Founded in 1905, the club competes in the Premier League, the top division of English football. Domestically, the club has won six league titles, eight FA Cups, five League Cups, and four FA Community Shields.",
        'country': 'England',
        'city': 'London',
        'liga': 'EPL',
        'founded': 1905,
        'Coach': 'Thomas Tuchel',
        'rating': 5,
        'stadion': 'Stamford Bridge',
        'webSait': 'https://www.chelseafc.com/'
    },
    {
        '_id': '7',
        'name': 'Crystal Palace F.C.',
        'image': '/images/CrystalPalaceEmblema.png',
        'description':
        "Crystal Palace Football Club is a professional football club based in Selhurst in the Borough of Croydon, South London, England, who currently compete in the Premier League, the highest level of English football. Although formally created as a professional outfit in 1905, the club's origins can be traced as far back as 1861.",
        'country': 'England',
        'city': 'London',
        'liga': 'EPL',
        'founded': 1905,
        'Coach': 'Patrick Vieira',
        'rating': 3.5,
        'stadion': 'Selhurst Park',
        'webSait': 'https://www.cpfc.co.uk/'
    },
    {
        '_id': '8',
        'name': 'Everton F.C.',
        'image': '/images/EvertonEmblema.png',
        'description':
        "Everton Football Club (/ˈɛvərtən/) is an English professional association football club based in Liverpool that currently competes in the Premier League, the top tier of English football. The club was a founder member of the Football League in 1888 and has competed in the top division for a record 118 seasons, having missed only four top-flight seasons (1930–31, 1951–52, 1952–53 and 1953–54).",
        'country': 'England',
        'city': 'Liverpool',
        'liga': 'EPL',
        'founded': 1878,
        'Coach': 'Frank Lampard',
        'rating': 4,
        'stadion': 'Goodison Park',
        'webSait': 'https://www.evertonfc.com/'
    },
    {
        '_id': '9',
        'name': 'Leeds United F.C.',
        'image': '/images/LeedsUnitedEmblema.png',
        'description':
        "Leeds United Football Club is an English professional football club based in the city of Leeds, West Yorkshire. The club was formed in 1919 following the disbanding of Leeds City by the Football League and took over their Elland Road stadium. The club currently competes in the Premier League, the top flight of English football, following promotion from the EFL Championship during the 2019—20 season.",
        'country': 'England',
        'city': 'Leeds',
        'liga': 'EPL',
        'founded': 1919,
        'Coach': 'Jesse Marsch',
        'rating': 3.5,
        'stadion': 'Elland Road',
        'webSait': 'https://www.leedsunited.com/'
    },
    {
        '_id': '10',
        'name': 'Leicester City F.C.',
        'image': '/images/LeicesterEmblema.png',
        'description':
        "Leicester City Football Club is a professional football club based in Leicester in the East Midlands, England. The club competes in the Premier League, the top tier of England's football league system, and plays its home matches at the King Power Stadium.",
        'country': 'England',
        'city': 'Leicester',
        'liga': 'EPL',
        'founded': 1884,
        'Coach': 'Brendan Rodgers',
        'rating': 4,
        'stadion': 'King Power Stadium',
        'webSait': 'https://www.lcfc.com/'
    },
    {
        '_id': '11',
        'name': 'Liverpool F.C.',
        'image': '/images/LiverpoolEmblema.png',
        'description':
        "Liverpool Football Club is a professional football club based in Liverpool, England, that competes in the Premier League, the top tier of English football.",
        'country': 'England',
        'city': 'Liverpool',
        'liga': 'EPL',
        'founded': 1892,
        'Coach': 'Jürgen Klopp',
        'rating': 5,
        'stadion': 'Anfield',
        'webSait': 'https://www.liverpoolfc.com/'
    },
    {
        '_id': '12',
        'name': 'Manchester City F.C.',
        'image': '/images/ManchesterCityEmblema.png',
        'description':
        "Manchester City Football Club is an English football club based in Manchester that competes in the Premier League, the top flight of English football. Founded in 1880 as St. Mark's (West Gorton), it became Ardwick Association Football Club in 1887 and Manchester City in 1894.",
        'country': 'England',
        'city': 'Manchester',
        'liga': 'EPL',
        'founded': 1880,
        'Coach': 'Pep Guardiola',
        'rating': 5,
        'stadion': 'Etihad Stadium',
        'webSait': 'https://www.mancity.com/'
    },
    {
        '_id': '13',
        'name': 'Manchester United F.C.',
        'image': '/images/ManchesterUnitedEmblema.png',
        'description':
        "Manchester United Football Club is a professional football club based in Old Trafford, Greater Manchester, England, that competes in the Premier League, the top flight of English football. Nicknamed the Red Devils, the club was founded as Newton Heath LYR Football Club in 1878, but changed its name to Manchester United in 1902. The club moved from Newton Heath to its current stadium, Old Trafford, in 1910.",
        'country': 'England',
        'city': 'Manchester',
        'liga': 'EPL',
        'founded': 1878,
        'Coach': 'Ralf Rangnick',
        'rating': 4.5,
        'stadion': 'Old Trafford',
        'webSait': 'https://www.manutd.com/'
    },
    {
        '_id': '14',
        'name': 'Newcastle United F.C.',
        'image': '/images/NewcastleUnitedEmblema.png',
        'description':
        "Newcastle United Football Club is an English professional football club based in Newcastle upon Tyne, that plays in the Premier League – the top flight of English football. The club was founded in 1892 by the merger of Newcastle East End and Newcastle West End. The team plays its home matches at St James' Park in the centre of Newcastle.",
        'country': 'England',
        'city': 'Newcastle',
        'liga': 'EPL',
        'founded': 1892,
        'Coach': 'Eddie Howe',
        'rating': 3.5,
        'stadion': "St James' Park",
        'webSait': 'https://www.nufc.co.uk/'
    },
    {
        '_id': '15',
        'name': 'Norwich City F.C.',
        'image': '/images/NorwichCityEmblema.png',
        'description':
        "Norwich City Football Club (also known as The Canaries or The Yellows) is an English professional football club based in Norwich, Norfolk. As of the 2021–22 season, the team competes in the Premier League, the top tier of English football, after winning the 2020–21 EFL Championship title. The club was founded in 1902.",
        'country': 'England',
        'city': 'Norvich',
        'liga': 'EPL',
        'founded': 1902,
        'Coach': 'Dean Smith',
        'rating': 3.5,
        'stadion': 'Carrow Road',
        'webSait': 'https://www.canaries.co.uk/'
    },
    {
        '_id': '16',
        'name': 'Southampton F.C.',
        'image': '/images/SouthamptonEmblema.png',
        'description':
        "Southampton Football Club (/saʊθˈ(h)æmptən/) is an English professional football club based in Southampton, Hampshire, which plays in the Premier League, the top tier of English football. One of the founding members of the Premier League, they have spent the majority of their history in the top flight since their first promotion to it in 1966; their longest continuous period was for 27 seasons between 1978 and 2005.",
        'country': 'England',
        'city': 'Southampton',
        'liga': 'EPL',
        'founded': 1885,
        'Coach': 'Ralph Hasenhüttl',
        'rating': 4,
        'stadion': "St Mary's Stadium",
        'webSait': "https://www.southamptonfc.com/"
    },
    {
        '_id': '17',
        'name': 'Tottenham Hotspur F.C.',
        'image': '/images/TottenhamHotspurEmblema.png',
        'description':
        "Tottenham Hotspur Football Club, commonly referred to as Tottenham (/ˈtɒtənəm/) or Spurs, is an English professional football club based in Tottenham, London, that competes in the Premier League, the top flight of English football. The team has played its home matches in the 62,850-capacity Tottenham Hotspur Stadium since April 2019, replacing their former home of White Hart Lane, which had been demolished to make way for the new stadium on the same site.",
        'country': 'England',
        'city': 'London',
        'liga': 'EPL',
        'founded': 1882,
        'Coach': 'Antonio Conte',
        'rating': 4.5,
        'stadion': 'Tottenham Hotspur Stadium',
        'webSait': 'https://www.tottenhamhotspur.com/'
    },
    {
        '_id': '18',
        'name': 'Watford F.C.',
        'image': '/images/WatfordEmblema.png',
        'description':
        "Watford Football Club is an English professional football club based in Watford, Hertfordshire. They play in the Premier League, the top tier of English football.",
        'country': 'England',
        'city': 'Watford',
        'liga': 'EPL',
        'founded': 1881,
        'Coach': '	Roy Hodgson',
        'rating': 3.5,
        'stadion': 'Vicarage Road',
        'webSait': 'https://www.watfordfc.com/'
    },
    {
        '_id': '19',
        'name': 'West Ham United F.C..',
        'image': '/images/WestHamUnitedEmblema.png',
        'description':
        "West Ham United Football Club is an English professional football club based in Stratford, East London that compete in the Premier League, the top tier of English football. The club plays at the London Stadium, having moved from their former home, the Boleyn Ground, in 2016.",
        'country': 'England',
        'city': 'London',
        'liga': 'EPL',
        'founded': 1895,
        'Coach': 'David Moyes',
        'rating': 4,
        'stadion': 'London Stadium',
        'webSait': 'https://www.whufc.com/'
    },
    {
        '_id': '20',
        'name': 'Wolverhampton Wanderers F.C.',
        'image': '/images/WolverhamptonWanderersEmblema.png',
        'description':
        "Wolverhampton Wanderers Football Club (/ˌwʊlvərˈhæmptən/ (audio speaker iconlisten)), commonly known as Wolves, is a professional association football club based in Wolverhampton, England. Formed as St. Luke's F.C.",
        'country': 'England',
        'city': ' Wolverhampton',
        'liga': 'EPL',
        'founded': 1877,
        'Coach': 'Bruno Lage',
        'rating': 4,
        'stadion': 'Molineux Stadium',
        'webSait': 'https://www.wolves.co.uk/'
    },
]

export default clubs